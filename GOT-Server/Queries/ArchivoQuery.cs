...
namespace GOT_Server.Queries
{
    public class ArchivoQuery
    {
        ...
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"SELECT `DireccionArchivo`, `DataArchivo`, `NombreArchivo`, `IDCommit` FROM `Archivo` NATURAL JOIN `Repositorie` WHERE `NameRepositorie` = @idrepo AND `NombreArchivo` = @name ORDER BY `IDCommit`;";
            cmd.Parameters.Add(new MySqlParameter
            {
                ParameterName = "@idrepo",
                DbType = DbType.String,
